...
void safe_main()
{
	...
	std::shared_ptr<myengine::Shader> _shader2 = myengine::Shader::Create("../resources/fragmentshaders/tritest.fs", "../resources/vertexshaders/tritest.vs", { "in_Position" }, _core->getResources());
	...
	//entity 4
	std::shared_ptr<myengine::Entity> _entity4 = _core->addEntity(glm::vec3(_core->getWidth() / 2, _core->getHeight() / 2, -5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(30.0f, 30.0f, 30.0f));

	_entity4->addComponent<myengine::TriangleRenderer>();

	_entity4->getComponent<myengine::TriangleRenderer>()->AttachShaderProgram(_shader2);

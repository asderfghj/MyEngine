...
void safe_main()
{
	...
	_Player->addComponent<frontier::AudioPlayer, std::shared_ptr<frontier::Sound>>(shootsound);
	...
	_missilePrefab->addComponent<frontier::Collider, glm::vec3>(glm::vec3(2.0f, 2.0f, 2.0f));

	_missilePrefab->addComponent<frontier::ProjectileBehavior, std::weak_ptr<frontier::Entity>>(_Player);
	...
	_core->getPooler("asteroidpooler")->Spawn(glm::vec3(30.0f, 0.0f, 0.0f));
